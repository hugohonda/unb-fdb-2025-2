...
SELECT 
    ...
    STDDEV(pf.pf_sem_impostos) AS desvio_padrao_preco,
...
SELECT 
    ...
    CASE 
        WHEN p.cap = 'Sim' AND pmvg.pmvg_sem_impostos IS NOT NULL 
        THEN pmvg.pmvg_sem_impostos
...
INNER JOIN tipos_produto tp ON p.id_tipo = tp.id_tipo
INNER JOIN precos_fabrica pf ON p.id_produto = pf.id_produto
LEFT JOIN precos_pmvg pmvg ON p.id_produto = pmvg.id_produto AND (pf.id_aliquota = pmvg.id_aliquota OR (pf.id_aliquota IS NULL AND pmvg.id_aliquota IS NULL))
WHERE p.comercializacao_2024 = 'Sim'
    AND pf.pf_sem_impostos IS NOT NULL
...
SELECT 
    ...
    SUM(pf.pf_sem_impostos) AS valor_total_pf,
    SUM(pmvg.pmvg_sem_impostos) AS valor_total_pmvg,
    SUM(pf.pf_sem_impostos - pmvg.pmvg_sem_impostos) AS economia_total_cap,
...
INNER JOIN precos_fabrica pf ON p.id_produto = pf.id_produto
INNER JOIN precos_pmvg pmvg ON p.id_produto = pmvg.id_produto AND (pf.id_aliquota = pmvg.id_aliquota OR (pf.id_aliquota IS NULL AND pmvg.id_aliquota IS NULL))
WHERE p.cap = 'Sim'
...
SELECT 
    ...
    CASE 
        ...
        WHEN pf.pf_sem_impostos > 10000 
        THEN 'ALERTA: Preço muito alto (acima de R$ 10.000)'
        WHEN pf.pf_sem_impostos IS NULL 
